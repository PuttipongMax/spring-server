...
public class RunRepository {
	...
	public void create(Run run){
		var updated = jdbcClient.sql(
			"INSERT INTO Run(id, title, started_on, completed_on, miles, location) values(?,?,?,?,?,?)"
				...
				List.of(
					run.id(), run.title(), run.startedOn(), run.completedOn(), 
					run.miles(), run.location().toString()
				)
			).update();
	...
	public void update(Run run, Integer id){
		var updated = jdbcClient.sql(
			...
			List.of(
				run.title(), run.startedOn(), run.completedOn(), 
 ...
 // private void init(){
 //  runs.add(new Run(
 //   1, "Monday Morning Run", LocalDateTime.now(), 
	// 		LocalDateTime.now().plus(30, ChronoUnit.MINUTES), 3, 
	// 		Location.INDOOR
 //  ));

 //  runs.add(new Run(
 //   2, "Wednesday evening Run", LocalDateTime.now(), 
